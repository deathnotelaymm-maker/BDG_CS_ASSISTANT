...
const OWNER_EMAIL = 'lacus.mm.ph@gmail.com';
...
async function route(request, env, url) {
  ...
  if (method === 'POST' && /^\/admin\/admin-users\/\d+\/password$/.test(path)) return json(await changeAdminPassword(env, idFromParts(path, 3), await readJson(request)), 200, env);
...
function getConnectionString(env) { if (!env.HYPERDRIVE?.connectionString) throw new Error('Missing Hyperdrive binding: HYPERDRIVE'); return env.HYPERDRIVE.connectionString; }
async function q(env, text, params = []) {
  if (!Array.isArray(params)) params = [];
  const client = new Client({ connectionString: getConnectionString(env), connectionTimeoutMillis: Number(env.DB_CONNECT_TIMEOUT_MS || 4000), query_timeout: Number(env.DB_QUERY_TIMEOUT_MS || 10000), statement_timeout: Number(env.DB_QUERY_TIMEOUT_MS || 10000) });
  try { await client.connect(); return await client.query(text, params); }
...
async function readJson(request) {
  ...
  try { return JSON.parse(text); } catch (jsonErr) {
    // Accept form bodies and malformed PowerShell/curl bodies like {email:admin@example.com,password:ChangeMe123!}
    try {
...
async function hashPassword(password) {
  ...
  return `sha256_salted$1$${saltB64}$${b64UrlEncode(digest)}`;
}
...
async function ensureOwnerAdmin(env, forceDefaultPassword = false) {
  ...
  const adminPassword = env.ADMIN_PASSWORD || DEFAULT_OWNER_PASSWORD;
  const passwordHash = await hashPassword(adminPassword);
  await ensureAdminAuthReady(env);

  // v0.6.6: there must be exactly one business owner email by default.
  // Existing admin@example.com owner rows are migrated to lacus.mm.ph@gmail.com.
  ...
  if (owner) {
    const needsPasswordRecovery = forceDefaultPassword || !owner.password_hash || hasUnsupportedPasswordHash(owner.password_hash);
    await q(env, `UPDATE admin_users SET name=COALESCE(NULLIF(name,''),'Owner'),role='owner',is_active=TRUE,password_hash=CASE WHEN $1::boolean THEN $2 ELSE password_hash END,updated_at=NOW() WHERE id=$3`, [needsPasswordRecovery, passwordHash, owner.id]);
...
async function changeAdminPassword(env, id, p = {}) { const password = p.password || p.new_password; if (!password || String(password).length < 8) bad('Password must be at least 8 characters'); const passwordHash = await hashPassword(password); await q(env, 'UPDATE admin_users SET password_hash=$1, updated_at=NOW() WHERE id=$2', [passwordHash, id]); await audit(env, 'change_password', 'admin_users', id, 'Password changed'); return { ok: true }; }
...
async function login(request, env) {
  // v0.6.3b: parse login body first and provide a safe owner recovery path before any DB bootstrap.
  // This prevents a broken/migrating admin_users table from locking the owner out.
  const p = await readJson(request);
  const adminEmail = String(env.ADMIN_EMAIL || OWNER_EMAIL).trim().toLowerCase();
  const adminPassword = env.ADMIN_PASSWORD || DEFAULT_OWNER_PASSWORD;
  const email = String(p.email || '').trim().toLowerCase();
  const password = String(p.password || '');

  if (email === adminEmail && password === adminPassword) {
    let owner = { id: 'owner-recovery', email: adminEmail, role: 'owner', name: 'Owner', is_active: true, twofa_enabled: false, session_version: 0 };
    // Best effort DB recovery. Login must still succeed even if DB migration fails.
    try {
      await ensureAdminAuthReady(env);
      await ensureOwnerAdmin(env, true);
      const found = (await q(env, 'SELECT * FROM admin_users WHERE lower(email)=lower($1) LIMIT 1', [adminEmail])).rows[0];
      if (found) owner = found;
      if (owner.twofa_enabled === true && !await verifyTotp(owner.twofa_secret, p.twofa_code || p.otp || p.code)) return json({ twofa_required: true, detail: '2FA code required' }, 202, env);
      const updated = await q(env, 'UPDATE admin_users SET last_login_at=NOW(), updated_at=NOW(), session_version=COALESCE(session_version,0)+1 WHERE lower(email)=lower($1) RETURNING *', [adminEmail]);
      owner = updated.rows[0] || owner;
      await audit(env, 'login_success', 'admin_users', owner.id || owner.email, `Owner login ${owner.email}`);
    } catch (err) {
      console.error('Owner recovery DB step failed but login will continue:', err?.message || err);
    }
    const token = await createToken(env, owner.email || adminEmail, owner.role || 'owner', Number(owner.session_version || 0));
    return json({ access_token: token, token_type: 'bearer', user: adminUserOut(owner), recovery_mode: owner.id === 'owner-recovery' }, 200, env);
  }
  ...
  let ok = false;
  if (user?.password_hash && !hasUnsupportedPasswordHash(user.password_hash)) ok = await verifyPassword(password, user.password_hash);
  if (!ok) {
    await audit(env, 'login_failed', 'admin_users', email || '', 'Invalid login attempt');
    return json({ detail: 'Invalid email or password' }, 401, env);
  }
  // Upgrade old supported PBKDF2 hashes after successful login.
  ...
  if (user.twofa_enabled === true && !await verifyTotp(user.twofa_secret, p.twofa_code || p.otp || p.code)) return json({ twofa_required: true, detail: '2FA code required' }, 202, env);
...
async function hmac(env, data) { const key = await crypto.subtle.importKey('raw', new TextEncoder().encode(env.JWT_SECRET || 'dev-secret-change-me'), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']); return new Uint8Array(await crypto.subtle.sign('HMAC', key, new TextEncoder().encode(data))); }
...
async function readToken(env, token) {
  ...
  if (expected !== sig) bad('Invalid token', 401);
  const payload = JSON.parse(new TextDecoder().decode(b64UrlDecode(p)));
  if (payload.exp < Math.floor(Date.now()/1000)) bad('Expired token', 401);

  // v0.6.3d stability fix:
  // Previous strict session_version validation caused immediate 401/logout when owner login
  // succeeded through recovery mode but the existing DB session_version had already changed.
  // Keep signature + expiry validation strict, but make the DB session check soft so admin pages
  // do not auto-logout right after login. Owner/Admin session controls still remain available
  // in the admin UI and can be hardened again after the DB migration is fully stable.
  try {
    ...
    if (row.is_active === false) bad('Admin account is disabled', 401);
    return { ...payload, email: row.email || payload.email, role: row.role || payload.role, twofa_enabled: row.twofa_enabled === true };
  } catch (err) {
...
async function changeOwnPassword(env, admin, p = {}) {
  ...
  if (!password || String(password).length < 8) bad('Password must be at least 8 characters');
