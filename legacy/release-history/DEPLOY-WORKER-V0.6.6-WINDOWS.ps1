...
Write-Host "Deploying Worker API v0.6.6..." -ForegroundColor Cyan
...
for ($i = 1; $i -le 10; $i++) {
  ...
  $url = "https://bdg-ai-help-api.bdgservice.workers.dev/health?v=066-$i"
  Write-Host "Health check attempt ${i}: $url" -ForegroundColor Cyan
  $health = curl.exe $url
  Write-Host $health
  if ($health -match '0.6.6-worker') { $ok = $true; break }
}
if (!$ok) { throw "Worker deploy check failed. Health does not show 0.6.6-worker." }
Set-Location $Root
Write-Host "Worker v0.6.6 deployed successfully." -ForegroundColor Green
