...
Write-Host "Deploying Worker API v0.6.2..." -ForegroundColor Cyan
...
Write-Host $health
if ($health -notmatch '0.6.2-worker') { throw "Worker deploy check failed. Health does not show 0.6.2-worker." }
Set-Location $Root
Write-Host "Worker v0.6.2 deployed successfully." -ForegroundColor Green
