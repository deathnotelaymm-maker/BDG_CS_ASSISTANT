...
export const API_BASE = (configuredApiBase || (import.meta.env.DEV ? "http://localhost:10000" : "")).replace(/\/$/, "");
...
export interface ChatContent {
  ...
  texts?: Record<string, { title?: string; online?: string; welcome?: string; welcome_title?: string; placeholder?: string; busy?: string }>;
...
export async function fetchChatContent(signal?: AbortSignal): Promise<ChatContent> {
  ...
  const combinedSignal = signal ? AbortSignal.any([signal, timeoutSignal]) : timeoutSignal;
  const res = await fetch(`${API_BASE}/chat/content`, { signal: combinedSignal });
