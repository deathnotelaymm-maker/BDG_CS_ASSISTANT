...
Write-Host ' Luke v1.17.4-R2 - Regression Contract Stabilization' -ForegroundColor Cyan
Write-Host '=====================================================================' -ForegroundColor Cyan
Write-Host ''
Write-Host 'No product feature or database migration is changed. Migration 047 remains current.' -ForegroundColor Cyan
Write-Host ''
Write-Host '[1/7] Verifying SHA-256 release payload...' -ForegroundColor Cyan
& (Join-Path $pkg 'VERIFY-V1.17.4-R2-PAYLOAD.ps1') -PackageRoot $pkg
if($LASTEXITCODE){throw 'Release payload verification failed.'}
Write-Host '[2/7] Verifying v1.17.4-R1 target baseline...' -ForegroundColor Cyan
& (Join-Path $pkg 'VERIFY-V1.17.4-R2-TARGET.ps1') -TargetRoot $target
...
$backup=Join-Path $parent ($leaf+'-backup-before-v1.17.4-R2-'+$stamp)
...
@{target=$target;existing_files=$existing;new_files=$newFiles;release='v1.17.4-R2';created_at=(Get-Date).ToUniversalTime().ToString('o')}|ConvertTo-Json -Depth 6|Set-Content -LiteralPath (Join-Path $backup 'BACKUP_MANIFEST.json') -Encoding UTF8
$rollback=@'
...
Write-Host '[4/7] Copying reviewed v1.17.4-R2 files...' -ForegroundColor Cyan
foreach($relative in @($manifest.changed_files)){
...
& (Join-Path $pkg 'VERIFY-V1.17.4-R2-INSTALLED.ps1') -PackageRoot $pkg -TargetRoot $target
if($LASTEXITCODE){throw 'Installed-file verification failed.'}
Write-Host '[6/7] Running safe local source checks...' -ForegroundColor Cyan
$node=Get-Command node -ErrorAction SilentlyContinue
if($node){
  Push-Location $target
  try{
    & node --check 'backend-api\scripts\support-foundation-regression-test.js';if($LASTEXITCODE){throw 'Human Support regression syntax failed.'}
    & node --check 'backend-api\scripts\v1.17.4-r2-regression-contract-stabilization-test.js';if($LASTEXITCODE){throw 'R2 regression syntax failed.'}
    ...
    & node 'backend-api\scripts\v1.17.4-r2-regression-contract-stabilization-test.js';if($LASTEXITCODE){throw 'v1.17.4-R2 regression-contract test failed.'}
    if(Test-Path -LiteralPath 'backend-api\node_modules\sanitize-html'){
      & npm --prefix backend-api run test:support;if($LASTEXITCODE){throw 'Dependency-backed Human Support foundation failed.'}
    }else{
      Write-Warning 'backend-api dependencies are not installed; full Human Support execution is deferred to GitHub CI.'
    }
...
@(
  ...
  'Release package: v1.17.4-R2 Regression Contract Stabilization',
  'Application version: 1.17.4',
  "Backup: $backup",
  'No migration is added or applied. Migration 047 remains current.',
  'Next migration: 048',
  'Recommended commit: v1.17.4-R2 regression contract stabilization'
)|Set-Content -LiteralPath (Join-Path $target 'INSTALL_RESULT_V1.17.4-R2.txt') -Encoding UTF8
Write-Host ''
Write-Host 'PASS: v1.17.4-R2 files installed and verified.' -ForegroundColor Green
Write-Host 'Review git diff, commit, push, and require the full GitHub CI before deployment.' -ForegroundColor Yellow
