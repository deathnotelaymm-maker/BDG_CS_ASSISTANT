...
const sheet = book.addWorksheet('Bank name change');
sheet.addRows([
...
const parsed = await parseKnowledgeWorkbook(buffer);
...
assert.ok(parsed.rows[0].warnings.some((warning) => warning.includes('Name was generated')));
...
assert.equal(draft.route_policy, 'answer_only');
assert.equal(draft.import_batch_id, 22);
assert.match(draft.knowledge_content, /Verify identity/);
