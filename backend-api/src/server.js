...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    jsonResponse(res, status, { ok: false, error: status >= 500 ? (error.publicMessage || 'Service temporarily unavailable') : error.message, code: error.code || (status >= 500 ? 'INTERNAL_ERROR' : 'BAD_REQUEST'), request_id: requestId, version: API_VERSION, ...(error.configuration_missing ? { configuration_missing: error.configuration_missing } : {}), ...(error.platform_context ? { platform_resolution: error.platform_context } : {}) }, { 'Cache-Control': 'no-store', 'X-Request-ID': requestId, ...(corsOrigin ? { 'Access-Control-Allow-Origin': corsOrigin } : {}) });
