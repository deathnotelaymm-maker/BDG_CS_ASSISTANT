...
export function importedRowToAiContentDraft(mapped = {}, platformKey = 'default', batchId = null) {
  ...
  return {
    ...
    rich_json_hi: '',
    rich_html_hi: '',
    qa_answer_html: clean(mapped.answer),
    qa_answer_json: '',
    qa_steps: clean(mapped.image_url || mapped.image_ref) ? [{ type: 'image', url: clean(mapped.image_url || mapped.image_ref), role: clean(mapped.image_role) || 'reference', alt: clean(mapped.image_alt), caption: clean(mapped.image_caption), placement: clean(mapped.image_placement) || 'after_answer' }] : [],
    localized_fields: { [normalizeImportLocale(mapped.locale)]: { ai_instruction: clean(mapped.instruction), visual_knowledge: clean(mapped.answer), visual_guide: clean(mapped.answer) } },
