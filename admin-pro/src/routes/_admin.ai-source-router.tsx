...
function AiSourceRouterPage() {
  const [router, setRouter] = useState<any>({ enabled: true, prompt_manager_enabled: true, source_order: sourceOptions.map((item) => item.value), enabled_sources: sourceOptions.map((item) => item.value), locale_strategy: "exact_then_default", max_candidates: 80 });
  ...
  const save = async () => {
    setSaving(true);
    try { setRouter(await api.updateAiSourceRouter({ enabled: router.enabled !== false, prompt_manager_enabled: router.prompt_manager_enabled !== false, source_order: router.source_order || sourceOptions.map((item) => item.value), enabled_sources: router.enabled_sources || sourceOptions.map((item) => item.value), locale_strategy: router.locale_strategy || "exact_then_default", max_candidates: Number(router.max_candidates || 80) })); message.success("AI source routing policy saved"); }
