...
title BDG v1.16.0-r4 Customer Service Installer Verification Hotfix
color 0B

echo ================================================================
echo   BDG v1.16.0-r4 - Customer Service Installer Verification Hotfix
echo ================================================================
echo.
echo This installer verifies the full r4 hotfix, creates a rollback backup,
...
set "BACKUP_BASE=%TARGET%-backup-before-v1.16.0-r4"
...
:backup_ready
if not exist "%PACKAGE_ROOT%\VERIFY-V1.16.0-R4-PAYLOAD.ps1" goto bad_release
if not exist "%PACKAGE_ROOT%\VERIFY-V1.16.0-R4-INSTALLED.ps1" goto bad_release
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%PACKAGE_ROOT%\VERIFY-V1.16.0-R4-PAYLOAD.ps1"
...
echo [3/6] Copying reviewed r4 hotfix files...
...
echo [4/6] Verifying the installed route repair safely...
set "BDG_VERIFY_TARGET=%TARGET%"
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%PACKAGE_ROOT%\VERIFY-V1.16.0-R4-INSTALLED.ps1"
...
:success
>"%PACKAGE_ROOT%\INSTALL_RESULT_V1.16.0-R4.txt" echo SUCCESS: v1.16.0-r4 hotfix copied and verified in %TARGET%
>>"%PACKAGE_ROOT%\INSTALL_RESULT_V1.16.0-R4.txt" echo Backup: %BACKUP%
echo.
echo ================================================================
echo   V1.16.0-R4 CUSTOMER SERVICE INSTALLER HOTFIX INSTALLED
echo ================================================================
echo.
echo Rollback backup: %BACKUP%
echo Commit: v1.16.0-r4 Fix Windows installer verification hang
goto done

:bad_release
echo ERROR: The r4 release is incomplete or a checksum failed.
goto done
:bad_target
...
goto done
:verify_error
echo ERROR: Files copied but r4 verification failed. Restore the rollback backup.
goto done
:done
echo.
pause
