...
foreach($relative in @($manifest.changed_files)){
  ...
  if($actual -ne ([string]$prop.Value).ToLowerInvariant()){throw "Installed SHA-256 mismatch: $relative"}
  $count++
}
Write-Host "PASS: verified $count installed v1.17.4-R2 files by SHA-256." -ForegroundColor Green
