...
if (-not $health.ok -or $health.version -ne '0.7.0a-render-neon') {
  throw "Render API is not ready or has the wrong version. Expected 0.7.0a-render-neon."
}
if ($health.database_provider -ne 'neon' -or $health.connection_mode -ne 'pooled-runtime') {
  throw "Render API is not using the expected Neon pooled runtime connection."
}
